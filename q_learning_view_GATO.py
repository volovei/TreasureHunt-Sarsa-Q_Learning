...
def main_menu():
    pygame.init()
    screen = pygame.display.set_mode((715, 864))
    ...
    background = pygame.transform.scale(background, (715, 864))

    while True:
        screen.blit(background, (0, 0))  # Desenhar a imagem de fundo

        title_text = font.render("Treasure Hunt", True, (0, 0, 0))
        play_text = font.render("Play", True, (0, 0, 0))
        quit_text = font.render("Quit", True, (0, 0, 0))

        # Desenhar retângulos atrás dos textos para melhorar a visibilidade
        #pygame.draw.rect(screen, (0, 0, 0), (140, 290, 435, 90))  # Retângulo do título
        #pygame.draw.rect(screen, (0, 0, 0), (255, 390, 200, 80))  # Retângulo do botão Play
        #pygame.draw.rect(screen, (0, 0, 0), (255, 490, 200, 80))  # Retângulo do botão Quit

        screen.blit(title_text, (150, 300))
        screen.blit(play_text, (275, 400))
        screen.blit(quit_text, (275, 500))

        mouse = pygame.mouse.get_pos()
        click = pygame.mouse.get_pressed()

        if 255 <= mouse[0] <= 455 and 400 <= mouse[1] <= 480:
            if click[0] == 1:
                return

        if 255 <= mouse[0] <= 455 and 500 <= mouse[1] <= 580:
            if click[0] == 1:
                pygame.quit()
                quit()
...
if __name__ == "__main__":
    main_menu()  # Exibir menu inicial antes de iniciar o jogo
